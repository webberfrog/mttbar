...
def get_hist_2d(hist_obj: hist.Hist, root_category: str, config_inst: od.Config):
    """
...
if __name__ == "__main__":
    ...
    ap.add_argument("--variables", nargs="+", default=("electron_eta", "electron_pt_rebin"), help="columnflow variable name; efficiency will be calculated as a function of this variable")
    ...
    variable_insts = [config_inst.get_variable(v) for v in args.variables]
    ...
    variable = "-".join(args.variables)
    hist_files = {
        "mc": [
            f"{store_path}/cf.MergeHistograms/{args.config}/{sample}/nominal/calib__skip_jecunc/sel__default_tgs/prod__default_tgs/weight__all_weights/{args.version}/hist__var_{variable}.pickle"
    ...
    for key in ("data", "mc"):
        ...
        h_pass = get_hist_2d(hists[key], "ele_trigger_pass", config_inst)
        h_fail = get_hist_2d(hists[key], "ele_trigger_fail", config_inst)    

        # add the histograms to get the total
        h_total = h_pass + h_fail

        # calculate the efficiency and uncertainty
    ...
    #     # Get axis names
    #     xaxis = h.axes[args.variable[0]]
    #     yaxis = h.axes[args.variable[1]]
    ...
    #     cbar.set_label(f"{label} efficiency")

    # # CMS label
    # mplhep.cms.label(ax=ax, fontsize=22, llabel="Work in progress")

    # # Axis labels
    # plt.xlabel(h.axes[args.variable[0]].label)
    # plt.ylabel(h.axes[args.variable[1]].label)
    ...
    for key in ("data", "mc"):
        ...
        if key == "mc":
            mplhep.hist2dplot(
                eff_val,
                h.axes[args.variables[0]].edges, 
                h.axes[args.variables[1]].edges,
                #histtype="step",
                label="MC",
            )
            # ax.bar(
            #     label="MC stat. unc.",
            #     x=[h.axes["electron_eta"].centers, h.axes["electron_pt_rebin"].centers],
            #     width=[h.axes["electron_eta"].edges[1:] - h.axes["electron_eta"].edges[:-1], h.axes["electron_pt_rebin"].edges[1:] - h.axes["electron_pt_rebin"].edges[:-1]],
    ...
    plt.ylabel(args.variables[1])
    plt.xlabel(args.variables[0])
    #plt.zlabel("Efficiency")
    # draw legend
    ...
    if "data" in efficiencies and "mc" in efficiencies:
        ...
        bin_centers = h.axes[args.variables[0]].centers
        bin_widths = h.axes[args.variables[0]].edges[1:] - h.axes[args.variables[0]].edges[:-1]
